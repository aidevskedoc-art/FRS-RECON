...
import {
  ...
  CONTRA_SCOPE_OPTIONS,
  CONTRA_AMBIGUITY_OPTIONS,
...
@Component({
  selector: 'app-cheque-matching-rules',
  standalone: true,
  imports: [
    FormsModule,
    TableModule,
...
export class ChequeMatchingRulesComponent {
  ...
  protected readonly contraAmbiguityOptions = CONTRA_AMBIGUITY_OPTIONS;
  ...
  protected readonly allRules = computed(() => this.matchingRules.chequeRules());
  ...
  protected actionLabel(value: string | null): string {
    ...
    if (value === 'CONTRA_ENTRY') return 'Contra Entry';
  ...
  protected conditionLines(rule: MatchingRule): string[] {
    if (rule.kind === 'CONTRA_ENTRY') return this.contraSummary(rule);
  ...
  /** Plain-English description of a contra rule for the Conditions column, which would otherwise read "No conditions". */
  protected contraSummary(rule: MatchingRule): string[] {
    const cfg = rule.contraConfig;
    if (!cfg) return ['Not configured'];
    const labelOf = (v: string) => CONTRA_KEY_FIELD_OPTIONS.find((o) => o.value === v)?.label ?? v;
    const lines = ['Match on ' + (cfg.keyFields ?? []).map(labelOf).join(' + ') + ' + amount'];
    if (Number(cfg.tolerance) > 0) lines.push(`Amount tolerance ₹${cfg.tolerance}`);
    lines.push(cfg.dateWindowDays === null ? 'Dates not compared' : `Within ${cfg.dateWindowDays} day(s) of the receipt`);
    lines.push(cfg.scope === 'DIVISION' ? 'Same unit only' : 'Any unit');
    return lines;
  }
  ...
  protected updateContraConfig(patch: Partial<ContraRuleConfig>): void {
    this.draft.update((d) => ({ ...d, contraConfig: { ...DEFAULT_CONTRA_CONFIG, ...d.contraConfig, ...patch } }));
  }
  ...
  protected save(): void {
    ...
    // A contra rule carries no conditions either, so the join-key requirement
    // below does not apply to it. Its own identity requirement is the cheque
    // number: a contra keyed on anything less is not an identity.
    if (d.kind === 'CONTRA_ENTRY') {
      const cfg = d.contraConfig;
      if (!cfg) return this.formError.set('Contra settings are required');
      if (!cfg.keyFields?.length) return this.formError.set('Choose at least one matching field');
      if (!cfg.keyFields.includes('chequeNo')) return this.formError.set('Cheque No must be one of the matching fields');
      if (!(Number(cfg.tolerance) >= 0)) return this.formError.set('Amount tolerance must be zero or more');
      this.submit({
        name: d.name.trim(),
        action: null,
        active: d.active,
  ...
  private submit(payload: MatchingRuleDraft): void {
    ...
    const request = this.editingId()
      ? this.matchingRules.updateChequeRule(this.editingId()!, payload)
      : this.matchingRules.addChequeRule(payload);
  ...
  protected toggleActive(rule: MatchingRule): void {
    this.matchingRules.updateChequeRule(rule.id, { active: !rule.active }).subscribe({
      error: (err) => this.listError.set(errorMessage(err)),
    });
  }

  protected deleteRule(id: string): void {
    this.matchingRules.removeChequeRule(id).subscribe({ error: (err) => this.listError.set(errorMessage(err)) });
  }
  ...
  protected moveRule(rule: MatchingRule, direction: -1 | 1): void {
    ...
    this.matchingRules.reorderChequeRules(list.map((r) => r.id)).subscribe({
