...
export class DiagMatchingRulesComponent {
  ...
  protected toggleActive(rule: MatchingRule): void {
    this.matchingRules.updateDiagRule(rule.id, { active: !rule.active }).subscribe({
      error: (err) => this.listError.set(errorMessage(err)),
    });
  }

  protected deleteRule(id: string): void {
    this.matchingRules.removeDiagRule(id).subscribe({ error: (err) => this.listError.set(errorMessage(err)) });
  }
  ...
  protected moveRule(rule: MatchingRule, direction: -1 | 1): void {
    ...
    this.matchingRules.reorderDiagRules(list.map((r) => r.id)).subscribe({
