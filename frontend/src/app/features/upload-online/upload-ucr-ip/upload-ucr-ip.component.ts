...
@Component({
  selector: 'app-upload-ucr-ip',
  standalone: true,
  imports: [ButtonModule],
  templateUrl: './upload-ucr-ip.component.html',
  styleUrl: './upload-ucr-ip.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UploadUcrIpComponent {
  private readonly router = inject(Router);
  private readonly ucrUpload = inject(UcrUploadService);
  private readonly auth = inject(AuthService);

  /** Set by the UPI & Card Feeds hub to suppress this screen's own page header. */
  ...
  protected upload(): void {
    ...
    from(files)
      .pipe(
        concatMap((file) =>
          this.ucrUpload.uploadUcrIp(file, this.auth.userId()).pipe(
