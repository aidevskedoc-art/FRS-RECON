...
const FORMAT_1_COLUMNS: ColumnDef[] = [
  ...
  { key: 'transactionRef1', header: 'Transaction Id' },
  { key: 'transactionRef2', header: 'Transaction Id2' },
  ...
  { key: 'cardAmount', header: 'Card Amount', kind: 'amount' },
...
const FORMAT_2_COLUMNS: ColumnDef[] = [
  { key: 'receiptNumber', header: 'Receipt Number' },
  { key: 'receiptDate', header: 'Receipt Date', kind: 'date' },
  { key: 'yhno', header: 'YHNO' },
  { key: 'diagNo', header: 'Diag Number' },
  { key: 'patientName', header: 'Patient Name' },
  { key: 'transactionRef1', header: 'Reference Number' },
  { key: 'transactionRef2', header: 'UPI Reference Number' },
  { key: 'transactionRef3', header: 'UPI Reference Number2' },
  { key: 'payType', header: 'PayType' },
...
@Component({
  ...
  styleUrl: './payment-batch-detail.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PaymentBatchDetailComponent {
  ...
  protected readonly columns = computed<ColumnDef[]>(() =>
    this.batch()?.uploadType === 'DIAG_PAYMENT' ? FORMAT_2_COLUMNS : FORMAT_1_COLUMNS,
  );

  constructor() {
    this.onlineUpload.fetchBatch(this.batchId).subscribe({
  ...
  protected deleteAll(): void {
    this.onlineUpload.deleteAllRecords(this.batchId).subscribe({
      next: () => {
        this.records.set([]);
        this.total.set(0);
  ...
  protected back(): void {
    this.router.navigate(['/upload-online/payments']);
  }
