...
@Component({
  selector: 'app-upload-refund-document',
  standalone: true,
  imports: [ButtonModule],
  templateUrl: './upload-refund-document.component.html',
  styleUrl: './upload-refund-document.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UploadRefundDocumentComponent {
  private readonly router = inject(Router);
  private readonly refunds = inject(RefundService);
  ...
  protected readonly uploadedBatches = signal<RefundBatch[]>([]);
  ...
  protected upload(): void {
    ...
    from(files)
      .pipe(
        concatMap((file) =>
          this.refunds.upload(file, this.auth.userId()).pipe(
            map((batch) => ({ fileName: file.name, batch, error: null as string | null })),
            catchError((err) => of({ fileName: file.name, batch: null as RefundBatch | null, error: errorMessage(err) })),
          ),
        ),
      )
      .subscribe({
  ...
  protected viewBatches(): void {
    this.router.navigate(['/upload-online/refund-documents']);
  }
