...
import { EasebuzzSettlementBatch } from '../../../core/models';

@Component({
  selector: 'app-upload-easebuzz-settlement',
  standalone: true,
  imports: [ButtonModule],
  templateUrl: './upload-easebuzz-settlement.component.html',
  styleUrl: './upload-easebuzz-settlement.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UploadEasebuzzSettlementComponent {
  ...
  protected readonly uploadedBatches = signal<EasebuzzSettlementBatch[]>([]);
  ...
  protected upload(): void {
    ...
    from(files)
      .pipe(
        concatMap((file) =>
          this.bankStatements.uploadEasebuzzSettlement(file, this.auth.userId()).pipe(
            map((batch) => ({ fileName: file.name, batch, error: null as string | null })),
            catchError((err) => of({ fileName: file.name, batch: null as EasebuzzSettlementBatch | null, error: errorMessage(err) })),
          ),
        ),
      )
      .subscribe({
  ...
  protected viewSettlements(): void {
    this.router.navigate(['/matched-rules/easebuzz-settlements']);
  }
